...
/**
 * This is a simple port of Steve Reid's SHA-1 code into Java. I've run his test
 * vectors through the code and they all pass.
 */
public final class SHA1 {
	...
	/*
	 * These functions are taken out of #defines in Steve's code. Java doesn't
	 * have a preprocessor so the first step is to just promote them to real
	 * methods. Later we can optimize them out into inline code, note that by
	 * making them final some compilers will inline them when given the -O flag.
	 */
	...
	private void R0(int data[], int v, int w, int x, int y, int z, int i) {
		data[z] += ((data[w] & (data[x] ^ data[y])) ^ data[y]) + blk0(i)
				+ 0x5A827999 + rol(data[v], 5);
		data[w] = rol(data[w], 30);
	}

	private void R1(int data[], int v, int w, int x, int y, int z, int i) {
		data[z] += ((data[w] & (data[x] ^ data[y])) ^ data[y]) + blk(i)
				+ 0x5A827999 + rol(data[v], 5);
		data[w] = rol(data[w], 30);
	}

	private void R2(int data[], int v, int w, int x, int y, int z, int i) {
		data[z] += (data[w] ^ data[x] ^ data[y]) + blk(i) + 0x6ED9EBA1
				+ rol(data[v], 5);
		data[w] = rol(data[w], 30);
	}
	...
	private void R4(int data[], int v, int w, int x, int y, int z, int i) {
		data[z] += (data[w] ^ data[x] ^ data[y]) + blk(i) + 0xCA62C1D6
				+ rol(data[v], 5);
		data[w] = rol(data[w], 30);
	}
	 ...
	 * 
	 * blk0() and blk() perform the initial expand. I got the idea of expanding
	 * during the round function from SSLeay
	 * 
	 * #define blk0(i) block->l[i] #define blk(i) (block->l[i&15] =
	...
	/**
	 * This is a test program for the SHA1 algorithm. It puts the three test
	 * vectors through the algorithm and prints out the results (they should
	 * match.) Then it runs the MessageDigest benchmark method to see how fast
	 * it is. on my P133 its about 110 - 120K bytes/second.
	 * 
	 * It then compares it to MD5, which is about 150K bytes/second.
	 * 
	 * The reference to MoreOutputStream can be deleted. This is a small class
	 * that opens a window to display the results. This works around Symantec
	 * Cafe's tendency to shut down the DOS window after showing the output, and
	 * the inability to scroll back in a DOS window.
	 */
	/*
	 * public static void main(String args[]) { int i; SHA1 s = new SHA1();
	 * 
	 ...
	 * 
	 * System.out.println("First test is 'abc'"); String z = "abc"; s.init();
	 * s.update((byte) 'a'); s.update((byte) 'b'); s.update((byte) 'c');
	 * s.finish(); System.out.println(s.digout());
	 * System.out.println("A9993E36 4706816A BA3E2571 7850C26C 9CD0D89D");
	 * 
	 * // "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq" // 84983E44
	 ...
	 * s.init(); s.updateASCII(z); s.finish(); System.out.println(s.digout());
	 * System.out.println("84983E44 1C3BD26E BAAE4AA1 F95129E5 E54670F1");
	 * 
	 * // A million repetitions of "a" // 34AA973C D4C4DAA4 F61EEB2B DBAD2731
	 * 6534016F
	 * 
	 * System.out.println("Last test is 1 million 'a' characters."); s.init();
	 * for (i = 0; i < 1000000; i++) s.update((byte) 'a'); s.finish();
